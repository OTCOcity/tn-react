...
import {API_POSTS_REQUEST_URL} from 'constants/API';

function APICall({endpoint, method, query, payload}) {
  return new Promise((resolve, reject) => {
    let r = request[method.toLowerCase()](`${API_POSTS_REQUEST_URL}${endpoint}`);
...
export default store => next => action => {
  ...
  const promise = APICall(pick(action[API_CALL], ['endpoint', 'method', 'query', 'payload']));
